...
// 创建一个新的操作对象
func New(q *QueueExchange) *RabbitMQNew {
	return &RabbitMQNew{
		queueName:    q.QuName,
		routingKey:   q.RtKey,
...
// 启动RabbitMQ客户端,并初始化
func (r *RabbitMQNew) Start() {
	// 开启监听生产者发送任务
	for _, producer := range r.producerList {
		go r.listenProducer(context.Background(), producer)
	}
	// 开启监听接收者接收任务
...
// 注册发送指定队列指定路由的生产者
func (r *RabbitMQNew) RegisterProducer(producer Producer) {
	r.producerList = append(r.producerList, producer)
}

// 发送任务
func (r *RabbitMQNew) listenProducer(ctx context.Context, producer Producer) {
	// 验证链接是否正常,否则重新链接
	if r.channel == nil {
		r.mqConnect()
	}
...
// 注册接收指定队列指定路由的数据接收者
func (r *RabbitMQNew) RegisterReceiver(receiver Receiver) {
	r.mu.Lock()
...
// 监听接收者接收任务
func (r *RabbitMQNew) listenReceiver(receiver Receiver) {
	// 处理结束关闭链接
	defer r.mqClose()
	// 验证链接是否正常
	if r.channel == nil {
